...
export const extractAuthQueryParamsFromVerificationURL = (inputUrl) => {
  ...
  try {
    ...
    const shortenedVerificationURL =
      validUrl.origin + inputUrl.split("verify/")[1]?.split("/")[1];
    const paramsPassed = new URL(shortenedVerificationURL).searchParams;
    return `role~${paramsPassed.get("auth_role")}~userType~${paramsPassed.get(
      "user_type"
    )}~portfolio~${paramsPassed.get("portfolio")}~username~${paramsPassed.get(
      "username"
    )}~org~${paramsPassed.get("org")}~product~${
      paramsPassed.get("product")
        ? paramsPassed.get("product")
        : paramsPassed.get(`${validUrl.origin}?product`)
    }`;
  } catch (error) {
    return "";
  }
  ...
  normality_analysis: {
    ...
    list1: {
      ...
      evidence_counter: {
        ...
        "Points in Range2": "no",
        "Points in Range3": "no",
        "Satifies sigmoid function whose mirror image will give bell shaped curve":
          "no",
...
export const DocumentProcessDetailReport = {
  ...
  nps_total_score: 120,
  max_total_score: 10,
  score_list: [10, 10, 4, 10, 10, 4, 10, 10, 4, 10, 10, 4],
  scale_specific_data: {
  ...
  normality_analysis: {
    ...
    list1: {
      ...
      Kurt: "Leptokurtic curve",
      ind_dev: 1.0,
      slope: [0.36789614249311026, 0.37455606159782767],
      slope_percentage_deviation: 0.25,
  ...
  central_tendencies:
    "Process Id already in use. Please enter a different Process Id & try again.",
};
