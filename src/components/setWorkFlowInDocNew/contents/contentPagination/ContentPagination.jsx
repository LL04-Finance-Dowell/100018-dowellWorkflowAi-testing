...
const ContentPagination = ({ pages, currentPage, updateCurrentPage }) => {
    ...
    useEffect(() => {
        ...
        if ( (!pages) || (initialPageRangeSet) || (isNaN(Number(pages))) || (!isNaN(Number(pages)) && pages < 1) ) return
    ...
    return <>
        <div className={styles.content__Pagination__Wrapper}>
                    ...
                    <div 
                        ...
                        onClick={
                            currentPagesRange?.startPage > 0 ? 
                                () => setCurrentPagesRange((prevRange => { return { ...prevRange, startPage: prevRange.startPage - 1, endPage: prevRange.endPage - 1 } }))
                            : 
                                () => {}
                        }
                    >
            ...
            <div className={styles.pages__Container}>
                {
                    React.Children.toArray(createArrayFromNumberRange(currentPagesRange?.startPage, currentPagesRange?.endPage).map(page => {
                        return <>
                            <span 
                                className={`${styles.page__Item} ${currentPage === page ? styles.active__Page : ''}`}
