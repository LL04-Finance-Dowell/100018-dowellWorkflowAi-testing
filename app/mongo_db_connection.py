...
def bulk_query_template_collection_modified(options, status=None):
    """
    Fetch templates based on specified options and status.

    Parameters:
        options (dict): Additional query parameters to filter templates.
        status (str): The status of templates to filter ("save templates" or "draft").

    Returns:
        list: A list of templates that match the specified options and status.
    """
    templates = get_data_from_data_service(*TEMPLATE_CONNECTION_LIST, "fetch", field=options)

    if status is not None:
        filtered_templates = [template for template in templates if template.get("template_state") == status]
...
def bulk_query_document_collection_modified(options, status=None):
    """
    Fetch documents based on specified options and status.

    Parameters:
        options (dict): Additional query parameters to filter documents.
        status (str): The status of documents to filter ("save documents" or "draft").

    Returns:
        list: A list of documents that match the specified options and status.
    """
    documents = get_data_from_data_service(*DOCUMENT_CONNECTION_LIST, "fetch", field=options)

    if status is not None:
        filtered_documents = [document for document in documents if document.get("document_state") == status]
