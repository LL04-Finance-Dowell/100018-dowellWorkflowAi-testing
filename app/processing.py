...
class Background:
    ...
    def processing(self):
        ...
        print(f"Process state just as it: {processing_state}")
        ...
        try:
            ...
            if no_of_steps > 0:
                for index, step in enumerate(steps):
                    if "stepDocumentCloneMap" in step:  # Check if key exists
                        for document_map in step.get("stepDocumentCloneMap"):
                            for k, v in list(document_map.items()):
                                if (isinstance(v, str) and single_query_clones_collection({"_id": v}).get("document_state") == "processing") or (isinstance(v, str) and single_query_clones_collection({"_id": v}).get("template_state") == "draft"):
                                    continue
                                elif (isinstance(v, str) and single_query_clones_collection({"_id": v}).get("document_state") == "finalized") or (isinstance(v, str) and single_query_clones_collection({"_id": v}).get("template_state") == "saved"):
                                    register_single_user_access(
